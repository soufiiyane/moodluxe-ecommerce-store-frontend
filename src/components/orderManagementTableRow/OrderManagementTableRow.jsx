...
const OrderManagementTableRow = ({order})=>{
    ...
    return(
        <tr className={"border-b-[1px] text-sm text-center text-black/70"}>
            ...
            <td>{order?.orderAdresse?.fullName}</td>
            <td>{order?.orderAdresse?.country}</td>
            <td>{order?.orderLineItemList?.length}</td>
            ...
            <td className={"text-center px-2"}>
                <div className={"flex items-center justify-around"}>
                    {invoiceLoading ?
                        <div>
                            <svg aria-hidden="true" className="mr-2 w-5 h-5 text-gray-400 animate-spin fill-white" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        :
                        <button onClick={handlePrintInvoiceClick}><AiFillPrinter className={"text-xl mx-1"}/></button>
                    }
                    <button className={"bg-gray-200 hover:bg-gray-300 transition-colors shadow-inner flex justify-center items-center rounded-lg py-2 px-3 gap-1 font-semibold"} onClick={()=>navigate("/admin/orders/"+order?.orderNumber)}><AiOutlineEye/></button>
                </div>
            </td>
        </tr>
    )
